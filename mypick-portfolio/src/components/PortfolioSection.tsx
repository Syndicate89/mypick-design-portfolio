    ...
    return (
        <section id="portfolio" className="pt-0 pb-32 bg-transparent relative overflow-hidden -mt-32 z-20">
            ...
            <div className="relative w-full group max-w-[100vw]">
                {/* Viewport */}
                <div className="overflow-hidden cursor-grab active:cursor-grabbing w-full pb-10" ref={emblaRef}>
                    <div className="flex touch-pan-y">
                        {projects.map((project, idx) => (
                            <div
                                key={`${project.id}-${idx}`}
                                className="flex-[0_0_85vw] md:flex-[0_0_60vw] lg:flex-[0_0_40vw] min-w-0 pl-6 lg:pl-8 first:pl-6"
                            >
                                <div className={`relative w-full aspect-[16/10] rounded-2xl overflow-hidden ${project.color} group/card border border-border shadow-sm`}>
                                    <img src={project.image} alt={project.name} className="absolute inset-0 w-full h-full object-cover mix-blend-multiply opacity-90 transition-transform duration-700 group-hover/card:scale-105 pointer-events-none" />
                                    ...
                                    {/* Overlay on Hover */}
                                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/card:opacity-100 transition-opacity duration-500 z-10 flex flex-col justify-end p-8 md:p-10">
